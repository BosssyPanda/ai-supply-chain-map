...
export function PageShell({
  ...
  return (
    <div className={cn('mx-auto w-full px-5 py-8 lg:px-8 lg:py-10', fullWidth ? 'max-w-[1780px]' : 'max-w-[1640px]', className)}>
      {children}
    </div>
  );
}
...
export function PageHeader({
  ...
  return (
    <header className={cn('mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between', className)}>
      <div>
        {eyebrow ? <p className="text-xs font-semibold uppercase tracking-[0.18em] text-accent">{eyebrow}</p> : null}
        <h1 className="font-display text-5xl leading-[1.05] text-foreground md:text-6xl">{title}</h1>
...
export function MainContentGrid({
  ...
  return <div className={cn('grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]', className)}>{children}</div>;
}
