...
const alertCategories: AlertCategory[] = [
  {
    ...
    currentState: 'Defined, not automated',
...
export function Alerts(): JSX.Element {
  ...
  const columns: ReportTableColumn<AlertCategory>[] = [
    ...
    { id: 'state', header: 'Current state', render: (row) => <span className="font-medium text-muted-foreground">{row.currentState}</span> },
  ...
  return (
    <PageShell>
      <HeroSection
        ...
        subtitle="A monitoring-style chapter for future changes in bottleneck severity, company roles, sources, and material risk. The page defines the research lanes without pretending live alert automation exists."
        stats={
          <>
            ...
            <StatCard icon={<Bell className="h-4 w-4" />} label="Live alerts" value="Not live" context="Automation is not active yet" />
      ...
      <MainContentGrid className="mt-8">
        <div className="space-y-6">
          <ContentSection>
            ...
            <div className="grid gap-4 p-5 md:grid-cols-2">
              <ChapterCard icon={<Shield className="h-4 w-4" />} title="Bottleneck severity changes" description="Detect material changes to critical, high, medium, or low severity fields." meta="Defined, not automated" />
              <ChapterCard icon={<Search className="h-4 w-4" />} title="Company role updates" description="Track changes to role, market segment, status, geography, confidence, or attached sources." meta="Defined, not automated" />
              <ChapterCard icon={<Database className="h-4 w-4" />} title="Source updates" description="Show new source rows, stale source dates, and reliability changes." meta="Defined, not automated" />
              <ChapterCard icon={<Zap className="h-4 w-4" />} title="Material risk updates" description="Monitor minerals, supplier mapping, geography, and material bottleneck fields." meta="Defined, not automated" />
            </div>
          </ContentSection>
          ...
          <ReportTable
            ...
            note="This chapter defines monitoring lanes only; no live alert feed, subscription system, or background automation is active."
          />
        </div>
        ...
        <RightRail>
          ...
          <InsightPanel title="Current state" eyebrow="Future monitoring">
            <p>There is no live alert engine, subscription system, or background monitor yet. Unavailable automation is framed as not live instead of shown as a broken feed.</p>
          </InsightPanel>
